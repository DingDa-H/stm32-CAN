...
stCanTxParamTdf stTxMsgArray[] =
{
	...
	{.ulID    = 0x234,.ucDLC   = 4,.pucData = s_Txdata,.ulIDE   = CAN_ID_STD,.ulRTR   = CAN_RTR_REMOTE,},
	...
	{.ulID    = 0x567,.ucDLC   = 4,.pucData = s_Txdata,.ulIDE   = CAN_ID_STD,.ulRTR   = CAN_RTR_DATA,},
	{.ulID    = 0x678,.ucDLC   = 4,.pucData = s_Txdata,.ulIDE   = CAN_ID_STD,.ulRTR   = CAN_RTR_DATA,},
};
...
void vCanService_Init(void)
{
	...
	filter.FilterIdHigh 			=(0x234<<5)|0x10;			/* 32位滤波器的高 16 位（对应标准 ID 的 11 位或扩展 ID 的高 16 位） */
	filter.FilterIdLow 				=0x345<<5 ;				/* 32位滤波器的低 16 位（对应扩展 ID 的低 18 位、IDE、RTR 位等） */
	filter.FilterMaskIdHigh 		=0x567<<5 ;				/* 与 FilterIdHigh 对应的掩码高 16 位（1 表示必须匹配，0 表示不关心） */
